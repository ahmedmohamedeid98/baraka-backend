<div style="width: 100%;">
    @php
        ...
        $vendorOrderId = is_callable($vendorOrderId) ? $vendorOrderId($getRecord()) : $vendorOrderId;
        
        // Define the main flow (excluding cancelled)
        $mainFlow = ['pending', 'processing', 'ready', 'collected'];
    ...
    <div style="display: flex; flex-direction: column; gap: 1.5rem;">
            ...
            <div style="display: flex; align-items: center; justify-content: space-between;">
                @foreach($mainFlow as $index => $status)
                    ...
                    <div style="display: flex; flex-direction: column; align-items: center; flex: 1; position: relative;">
                        ...
                        <button
                            type="button"
                            wire:click="$dispatch('openVendorStatusModal', { status: '{{ $status }}', vendorOrderId: {{ $vendorOrderId }} })"
                            style="{{ $buttonStyles }}"
        ...
        @if($currentStatus !== 'cancelled' && in_array('cancelled', $statusFlow[$currentStatus]['next'] ?? []))
            <div style="padding-top: 1rem; border-top: 1px solid #e5e7eb;">
                <div style="display: flex; align-items: center; justify-content: center;">
                    <button
                        type="button"
                        wire:click="$dispatch('openVendorStatusModal', { status: 'cancelled', vendorOrderId: {{ $vendorOrderId }} })"
        ...
        <div style="margin-top: 1rem; padding: 0.75rem; background-color: #f9fafb; border-radius: 0.5rem;">
            <p style="font-size: 0.75rem; color: #4b5563;">
                <span style="font-weight: 600;">Instructions:</span> Click on any <span style="color: #10b981; font-weight: 600;">green</span> status to update this vendor order. A reason is required for accountability.
            </p>
        </div>
    </div>
</div>

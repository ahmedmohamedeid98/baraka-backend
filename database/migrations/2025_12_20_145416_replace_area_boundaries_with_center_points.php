...
return new class extends Migration
{
    ...
    public function up(): void
    {
        Schema::table('areas', function (Blueprint $table) {
            // Drop old boundary columns and index
            $table->dropIndex('area_boundaries_index');
            $table->dropColumn(['from_latitude', 'to_latitude', 'from_longitude', 'to_longitude']);
            
            // Add new center points JSON column
            // Format: [{"latitude": 30.0444, "longitude": 31.2357, "radius_km": 4}, ...]
            $table->json('center_points')->nullable()->after('delivery_fee');
        });
    }
    ...
    {
        Schema::table('areas', function (Blueprint $table) {
            $table->dropColumn('center_points');
